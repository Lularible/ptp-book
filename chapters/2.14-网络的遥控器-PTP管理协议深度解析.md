...
│ reserved | actionField (1字节)：高4位保留/低4位动作类型 │
│ reserved (1字节)：保留                              │
│ ──────────────────────────────────────────────────│
│ MANAGEMENT TLV (可变)                              │
│   - tlvType (2字节)：0x0001                        │
...
响应返回：
目标设备发送响应，startingBoundaryHops = 3 - 1 = 2，boundaryHops = 2
BC2转发，boundaryHops = 1
BC1转发，boundaryHops = 0
管理节点接收，boundaryHops = 0
```
...
注意：
响应由目标设备重新发起
startingBoundaryHops = 请求的(startingBoundaryHops - boundaryHops)
boundaryHops从startingBoundaryHops起逐跳递减
到达管理节点时boundaryHops = 0
```
