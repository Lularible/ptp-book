...
- **分类**：动态
- **含义**：
  - leap59=TRUE：当天（当前 UTC 日）最后一分钟将有59秒（负闰秒）
  - leap61=TRUE：当天（当前 UTC 日）最后一分钟将有61秒（正闰秒）
...
问题2：我离主时钟多远？
答案：currentDS.stepsRemoved = 2（计的是通信路径跳数：A→B→C 共 2 跳；该拓扑只有 1 个边界时钟 B）
