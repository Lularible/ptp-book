...
B进入PRE_MASTER后，等待`qualificationTimeout`。该超时等于 **N × announceInterval**：

- 如果推荐状态由 **M1/M2 决策**产生，N = 0，超时即为 0；
- 如果推荐状态由 **M3 决策**产生（非主时钟设备上的MASTER端口），N = stepsRemoved + 1，即 (stepsRemoved + 1) × announceInterval。
